...
const generateForEveryScale = (inputPath, width, height, fileName, outputFile, fileType, type) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    for (let key in commonPngAndJpg_1.AndroidImageSize) {
        ...
        if (fileType === types_1.AndroidFileType.Png) {
            yield (0, commonPngAndJpg_1.convertPng)(inputPath, Math.floor(height * scale), Math.floor(width * scale), (0, common_js_1.getOutpoutFilePath)(outputFile + commonPngAndJpg_1.AndroidImageSize[key].path, fileName), 1);
        }
        else if (fileType === types_1.AndroidFileType.Jpeg) {
            yield (0, commonPngAndJpg_1.convertJpg)(inputPath, Math.floor(height * scale), Math.floor(width * scale), (0, common_js_1.getOutpoutFilePath)(outputFile + commonPngAndJpg_1.AndroidImageSize[key].path, fileName), 1);
        }
